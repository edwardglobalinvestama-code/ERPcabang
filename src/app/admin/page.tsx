...
export default function AdminPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [dashboardRes, branchesRes, rolesRes, staffRes] = await Promise.all([
            ...
            <form onSubmit={handleCreateStaff} className="mb-6 p-4 bg-white/[0.03] rounded-xl border border-white/5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input value={staffForm.pin} onChange={(e) => setStaffForm({ ...staffForm, pin: e.target.value.replace(/\D/g, "") })} className="input-field text-sm" placeholder="PIN (angka)" maxLength={6} inputMode="numeric" />
                </div>
                <div className="space-y-1.5">
                  ...
                  {filteredStaff.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-gray-500">
                        {staffSearch ? "Tidak ada staff yang cocok" : "Belum ada staff"}
                      </td>
                    </tr>
                  ) : (
                    filteredStaff.map((s) => (
